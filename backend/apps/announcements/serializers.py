...
class AnnouncementReplySerializer(serializers.ModelSerializer):
    ...
    def get_is_voice_note(self, obj):
        return bool(obj.voice_note_url)
...
class AnnouncementDetailSerializer(serializers.ModelSerializer):
    author = serializers.SerializerMethodField()
    replies = AnnouncementReplySerializer(many=True, read_only=True)
    ...
    def get_targeted_members(self, obj):
        ...
        return [{"id": m.id, "full_name": m.full_name} for m in obj.targeted_members.all()]
...
class AnnouncementCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        targeted_members = validated_data.pop('targeted_members', [])
        ...
        if targeted_members and not announcement.is_for_all:
